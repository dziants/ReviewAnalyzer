 ...
 * - The most advised way to do this is to use a third party open-source/free-ware  csv parsing package.
 ...
 * Net time taken to code approx. 3 hours
...
public class ReviewAnalyzer {
    ...
    public static void main(String[] args) {
        ReviewAnalysisEngine engine = new ReviewAnalysisEngine();
        try {
            System.out.println("Starting Review Analysis.");
            ReviewAnalysisEngine.AnalysisResults results = engine.doAnalyze();
            System.out.println();
            System.out.println("Here are the results:-");
            System.out.println();
            System.out.println("Top "+ReviewAnalysisEngine.MAX_NUM_RESULTS+ " users who gave reviews:");
            System.out.printf("%-50s %-20s", "User Name", "Number of Reviews");
            System.out.println();
            int maxUserReviews = ReviewAnalysisEngine.MAX_NUM_RESULTS;
            for (ReviewAnalysisEngine.AnalysisResults.StringNumTimesPair result : results.mostActiveUsers) {
                System.out.printf("%-50s %4d", result.getStr(), result.getNumTimes());
                System.out.println();
                maxUserReviews--;
                if (maxUserReviews<=0){
            ...
            System.out.println();
            System.out.println("Top "+ReviewAnalysisEngine.MAX_NUM_RESULTS+ " food items that have had reviews:");
            System.out.printf("%-50s %-20s", "Item Number", "Number of Reviews");
            System.out.println();
            for (ReviewAnalysisEngine.AnalysisResults.StringNumTimesPair result : results.mostCommentedItems) {
                System.out.printf("%-50s %4d", result.getStr(), result.getNumTimes());
                System.out.println();
           }
            System.out.println();
            System.out.println("Top "+ReviewAnalysisEngine.MAX_NUM_RESULTS+ " most used words that have appeared in reviews:");
            System.out.printf("%-50s %-20s", "Word", "Number of Times");
            System.out.println();
            for (ReviewAnalysisEngine.AnalysisResults.StringNumTimesPair result : results.mostUsedWords) {
                System.out.printf("%-50s %6d", result.getStr(), result.getNumTimes());
                System.out.println();
            }
            System.out.println();
        } catch (Throwable t) {
            System.out.println ("An exception was thrown during processing.");
        } finally {
            try {
                engine.closeDown();
            } catch (Throwable t) {}
